...
export default function App() {
    const [properties, setProperties] = useLocalStorage('properties', getInitialProperties());
    ...
    const addLead = (newLead) => { setLeads(prev => [...prev, newLead]); showToast('¡Gracias por tu interés! Nos pondremos en contacto pronto.'); };
    const updateLeadStatus = (id, status) => { setLeads(prev => prev.map(lead => lead.id === id ? { ...lead, status } : lead)); };
    const deleteLead = (id) => { setLeads(prev => prev.filter(lead => lead.id !== id)); showToast('Cliente eliminado.'); };
    ...
    const saveProperty = async (formData, imageFiles, brochureFile) => {
        ...
        // Procesa y convierte las imágenes si se subieron nuevas
        ...
        // Procesa y convierte el folleto si se subió uno nuevo
        if (brochureFile) {
        ...
        setProperties(prev => {
            if (finalData.id) {
                // Si se está editando, se mantienen las imágenes anteriores si no se suben nuevas
                const originalProperty = prev.find(p => p.id === finalData.id);
                if (!finalData.images) finalData.images = originalProperty.images;
                if (!finalData.brochure) finalData.brochure = originalProperty.brochure;
                return prev.map(p => p.id === finalData.id ? finalData : p);
            } else {
                // Si es una propiedad nueva y no se suben imágenes, se usa un placeholder
                if (!finalData.images) {
                    finalData.images = ['https://placehold.co/600x400/cccccc/ffffff?text=Nueva+Propiedad'];
                }
                return [...prev, { ...finalData, id: Date.now() }];
            }
        });
    ...
    const renderPage = () => {
        ...
        switch (currentPage) {
            ...
            default: return <HomePage onAddLead={addLead} />;
        }
    };
